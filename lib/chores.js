...
export function generateAssignments(dateStr, { force = false } = {}) {
  ...
  const tx = db.transaction(() => {
    ...
    for (const chore of pooled) {
      ...
      // HARD CAP: only assign to a child whose budget still has room for this chore.
      // Among those who fit, pick the one with the most remaining capacity.
      let best = null;
      ...
      ordered.forEach((cid, idx) => {
        ...
        if (chore.minutes > capacity) return; // doesn't fit this child's remaining budget
        const score = capacity - idx * 0.001; // tie-break by rotated order
      ...
      if (best == null) {
        // Nobody has budget room — leave it unassigned for the parent to hand out.
        db.prepare(
