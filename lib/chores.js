...
// The deadline for a day's batch is noon of the FOLLOWING day, giving a full
// noon-to-noon window. (Set the server TZ env var so "noon" is your local noon.)
export function deadlineFor(dateStr) {
  const [y, m, d] = dateStr.split('-').map(Number);
  const dl = new Date(y, m - 1, d, 12, 0, 0, 0);
