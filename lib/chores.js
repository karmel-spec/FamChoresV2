...
export function generateAssignments(dateStr, { force = false } = {}) {
  ...
  const tx = db.transaction(() => {
    if (force) {
      ...
      db.prepare('DELETE FROM unassigned_chores WHERE date = ?').run(dateStr);
    }
    ...
    const markUnassigned = (chore) =>
      db
        .prepare(
          `INSERT OR IGNORE INTO unassigned_chores (date, chore_id, title, minutes, difficulty)
           VALUES (?, ?, ?, ?, ?)`
        )
        .run(dateStr, chore.id, chore.title, chore.minutes, chore.difficulty);

    // HARD CAP pass: assign each chore to the eligible child with the most room,
    // never exceeding budgetOf(child). Chores that fit nobody go unassigned.
    const assignPass = (chores, loadMap, budgetOf, source) => {
      for (const chore of chores) {
        const ordered = rotate(eligibleChildIdsFor(db, chore.id, childIds), dayIndex);
        let best = null;
        let bestScore = -Infinity;
        ordered.forEach((cid, idx) => {
          const capacity = budgetOf(cid) - loadMap[cid];
          if (chore.minutes > capacity) return;
        ...
        if (best == null) {
          markUnassigned(chore);
          continue;
        }
        insertAssignment(db, dateStr, best, chore, source);
        loadMap[best] += chore.minutes;
      }
    };

    const regularBudget = (cid) => children.find((c) => c.id === cid)?.minute_budget || 0;

    // forced-assignment chores first (these bypass the budget cap)
    const load = {};
    childIds.forEach((id) => (load[id] = 0));
    for (const chore of rotating.filter((c) => c.assigned_child_id)) {
    ...
    // Saturday gets an extra yard-work queue from a chosen category, on top of the
    // regular daily budget. On Saturday that category is reserved for the extra queue.
    const isExtraDay = weekday === 6; // Saturday
    const extraCategory = getSetting('saturday_extra_category', 'Outdoor');
    const extraMinutes = parseInt(getSetting('saturday_extra_minutes', '60'), 10) || 0;
    const extraActive = isExtraDay && extraMinutes > 0;

    const regularPool = extraActive
      ? pooled.filter((c) => (c.category || '') !== extraCategory)
      : pooled;
    assignPass(regularPool, load, regularBudget, 'rotating');

    if (extraActive) {
      const yardLoad = {};
      childIds.forEach((id) => (yardLoad[id] = 0));
      const yardPool = pooled.filter((c) => (c.category || '') === extraCategory);
      assignPass(yardPool, yardLoad, () => extraMinutes, 'yard');
    }
...
export function unassignedForDate(dateStr) {
  return getDb()
    .prepare('SELECT * FROM unassigned_chores WHERE date = ? ORDER BY minutes DESC, title')
    .all(dateStr);
}

// Manually give an unassigned chore to a child (overrides the budget cap).
export function assignChoreToChild(dateStr, childId, choreId) {
  const db = getDb();
  ...
  db.prepare(
    `INSERT OR IGNORE INTO assignments (date, child_id, chore_id, source, minutes, difficulty, title)
     VALUES (?, ?, ?, 'rotating', ?, ?, ?)`
  ).run(dateStr, childId, choreId, chore.minutes, chore.difficulty, chore.title);
  db.prepare('DELETE FROM unassigned_chores WHERE date = ? AND chore_id = ?').run(dateStr, choreId);
}
...
export function dayStatsForChild(childId, dateStr) {
  ...
  // Minutes reflect the rotating "family contribution" + Saturday yard chores,
  // not the fixed daily priority chores.
  const rotating = rows.filter((r) => r.source !== 'priority');
