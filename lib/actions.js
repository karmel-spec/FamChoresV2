...
export async function assignUnassignedChore(formData) {
  const childId = Number(formData.get('child_id'));
  const choreId = Number(formData.get('chore_id'));
  if (childId && choreId) assignChoreToChild(todayStr(), childId, choreId);
  revalidateAll();
}
...
export async function setTimezone(formData) {
  const tz = (formData.get('timezone') || '').toString();
  if (isValidTimezone(tz)) setSetting('timezone', tz);
  revalidateAll();
}

export async function setSaturdayYard(formData) {
  const minutes = Math.max(0, parseInt(formData.get('saturday_extra_minutes'), 10) || 0);
  const category = (formData.get('saturday_extra_category') || 'Outdoor').toString().trim() || 'Outdoor';
  setSetting('saturday_extra_minutes', String(minutes));
  setSetting('saturday_extra_category', category);
  revalidateAll();
}
...
// ---------- site-wide gate ----------
export async function siteLogin(formData) {
  const entered = (formData.get('password') || '').toString().trim();
  if (entered && entered.toLowerCase() === getSitePassword().toLowerCase()) {
    cookies().set(SITE_COOKIE, expectedSiteToken(), COOKIE_OPTS);
    const dest = (formData.get('next') || '/').toString();
    redirect(dest.startsWith('/') ? dest : '/');
  }
  redirect('/site-login?e=1');
}
