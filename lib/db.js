...
function getDb() {
  ...
  migrate(db);
  seedIfEmpty(db);
  runOneTimeCleanups(db);
  return db;
}

// Idempotent, flag-guarded data fixups for databases that were seeded before a change.
function runOneTimeCleanups(db) {
  const flag = (key) => db.prepare('SELECT value FROM settings WHERE key = ?').get(key);
  const setFlag = (key) =>
    db.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').run(key, '1');

  // Remove the original pet-related chores.
  if (!flag('cleanup_pets_v1')) {
    const ids = db
      .prepare("SELECT id FROM chores WHERE category = 'Pets'")
      .all()
      .map((r) => r.id);
    const tx = db.transaction(() => {
      for (const id of ids) {
        db.prepare('DELETE FROM chore_eligibility WHERE chore_id = ?').run(id);
        db.prepare('DELETE FROM assignments WHERE chore_id = ?').run(id);
        db.prepare('DELETE FROM unassigned_chores WHERE chore_id = ?').run(id);
        db.prepare('DELETE FROM chores WHERE id = ?').run(id);
      }
    });
    tx();
    setFlag('cleanup_pets_v1');
  }
}
